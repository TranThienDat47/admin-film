...
const videoSrc = 'http://localhost:5000/api/video/stream/661d4da8768c13e7c844820c?mode=m3u8';
...
const Watch = ({ width, height }) => {
   ...
   const handleAutoProgress = () => {
      ...
      setTimeout(() => {
         curIntervalRef.current = setInterval(() => {
            ...
            progressBall.current.style.transform = `translateX(calc(${moveProgress * 100}% - var(--size-ball-progress) / 2))`;
   ...
   const handlePreviewVideo = useCallback((e) => {
      ...
      curImgWrapperRef.current.style.transform = `translateX(calc(${moveProgress * 100}% - ${144 / 2}px))`;

      if (curImgRef.current.getBoundingClientRect().left <= progressRefRef.current.getBoundingClientRect().left) {
         curImgWrapperRef.current.style.transform = `translateX(0)`;
      } else if (curImgRef.current.getBoundingClientRect().right >= progressRefRef.current.getBoundingClientRect().right) {
         ...
         if (
            ...
            for (let i = 0; i < videoRef.current.buffered.length; i++) {
               if (videoRef.current.buffered.start(i) <= videoRef.current.currentTime && videoRef.current.buffered.end(i) >= videoRef.current.currentTime) {
                  ...
                  progressBuffered.current.style.transform = `scaleX(${videoRef.current.buffered.end(i) / videoRef.current.duration})`;
                  break;
               }
            }
         }
   ...
   const handleMouseMoveVolume = (e) => {
      if (dragVolumeRef.current) {
         ...
         const widthVolume = volumeProgressRef.current.getBoundingClientRect().width - widthVolumeBall;
         ...
         volumeProgressBallRef.current.style.transform = `translateX(calc(${moveVolume * 100}% + var(--size-ball-progress) / -2))`;
      }
   };
   ...
   const handleKeyVolume = (e) => {
      if (focusVideoRef.current || screenStateRef.current === 1) {
         if (e.keyCode === 38) {
            ...
            volumeProgressBallRef.current.style.transform = `translateX(calc(${volumeCurRef.current * 100}% + var(--size-ball-progress) / -2))`;
         }
   ...
   useEffect(() => {
      videoRef.current.ontimeupdate = () => {
         if (videoRef.current && videoRef.current.currentTime) timeCurrentRef.current.innerHTML = `${convertTime(videoRef.current.currentTime)}`;

         if (videoRef.current && !videoRef.current.paused) setPlay(1);
      };

      const loadUser = async () => {
         const response = await axios.get(`${apiUrl}/video/thumbnail/6602fd24c34554fae23b7887`);
         ...
         console.log(previewListRef.current);
      };
      loadUser();
   }, []);
   ...
   useEffect(() => {
      ...
      watchRef.current.onmousemove = () => {
         if (!videoRef.current.paused && !dragVideoRef.current && !dragVolumeRef.current && !hoverProgress.current) {
            setShowControl(1);
   ...
   useEffect(() => {
      ...
      videoRef.current.oncanplay = (e) => {
         ...
         progressBuffered.current.style.transform = `scaleX(${videoRef.current.buffered.end(tempBufferedRef.current) / videoRef.current.duration})`;
      };
   ...
   useEffect(() => {
      ...
      progressRefRef.current.onmouseover = (e) => {
         ...
         if (!curImgWrapperRef.current.contains(e.target)) curImgWrapperRef.current.style.display = 'block';
   ...
   return (
      <div ref={watchRef} className={cx('wrapper')}>
         <div ref={wrapperVideoRef} className={cx('watch')}>
            <ReactHlsPlayer src={videoSrc} autoPlay={true} controls={false} width="100%" height="auto" playerRef={videoRef} tabIndex="0" />
                     ...
                     <div ref={volumeProgressRef} className={cx('volume-progress')}>
                        <div>
                           <div ref={volumeProgressMainRef} className={cx('volume-progress__main')}>
                              <div ref={volumeProgressCurrentRef} className={cx('volume-progress__current')}></div>
                           </div>
                           <div ref={volumeProgressBallRef} className={cx('volume-progress__ball-wrapper')}>
                              <div ref={ballVolumeRef} className={cx('volume-progress__ball')}></div>
                           </div>
                        </div>
                     </div>
